...
def spherical_lens(angles, metric, r_obs, r_src,
    ...
    if r_src < r_obs:
        warn('performing lensing for sources closer to singularity'
    ...
    for theta in wrap(angles, complement=True):
        ...
        if np.abs(theta) > np.pi/2:
            ...
            if not res[1].converged:
                warn('unresolvable impact parameter encountered',
                     RuntimeWarning, stacklevel=2)
                yield np.NaN
                continue
            ...
            try:
                ...
                warn('singularity encountered at impact parameter',
                     RuntimeWarning, stacklevel=2)
                pass

            if not ((S2R2_impact is np.NaN)
